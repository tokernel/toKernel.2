...
class session_lib {
	...
	public function __construct() {
		...
		if(!isset($_SESSION[$this->sp . '_initiated'])) {
			$this->regenerate();
			$_SESSION[$this->sp . '_initiated'] = true;
		}
		...
		if(isset($_SESSION[$this->sp . '_HTTP_USER_AGENT']) and $_SESSION[$this->sp . '_HTTP_USER_AGENT'] != md5($user_agent)) {
			...
			$_SESSION[$this->sp . '_HTTP_USER_AGENT'] = md5($user_agent);
		}
	...
	/**
	 * Get session value by item, sub-item.
	 *
	 * @access public
	 * @param string $item
	 * @param string $section
