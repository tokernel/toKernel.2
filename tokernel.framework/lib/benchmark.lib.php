 ...
 * Benchmark library for application debugging.
 *
...
class benchmark_lib {
	...
	public function between($owner_start, $owner_end) {
		...
		if(!isset($this->buffer[$owner_end]['end'])) {
			return round(round(microtime(true), 3) - $this->buffer[$owner_start]['start']);
		}
		
		return round(($this->buffer[$owner_end]['end'] - $this->buffer[$owner_start]['start']), 3);
	...
	public function elapsed($owner) {
		...
		if(!isset($this->buffer[$owner]['duration'])) {
			return round(round(microtime(true), 3) - $this->buffer[$owner]['start'], 3);
		}
