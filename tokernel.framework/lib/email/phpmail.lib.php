 ...
 * PHPMAIL - Email sender class library.
 *
 ...
 */
class phpmail_lib extends email_base_lib {
	...
	public function send() {
		...
		switch ($content_type) {
			...
			case 'plain':
				...
				$this->config['final_body'] = $this->config['body'];
		...
		if (count($this->file_attachments) > 0) {
			...
			$this->config['final_body'] = $body;
		}

		if (!mail($this->recipients, $this->config['Subject'], $this->config['final_body'], $this->config['email_header'])) {
			$this->debug_log('Failed to send mail');
			return false;
		} else {
